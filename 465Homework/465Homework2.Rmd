...
Want O(logn + logm) or O(log(nm)) running time.


```{}

KSmall(int[] A, int[] B, int k) {
l
}
...
We know that in this algorithm we have a recurrence that divides the problem into 1 subproblme of size $\sqrt{n}$. Each problem at that level ahs the work of $\Theta(loglog(n))$ implying the total work on each level is $\Theta(loglog(\sqrt{n}))$. We also know that the total number of levels would be on the order of loglog(n). Knowing the height of the tree and work on each level we can conclude a running time of $\Theta(loglog^2(n))$.
